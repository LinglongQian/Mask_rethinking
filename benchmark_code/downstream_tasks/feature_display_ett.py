...
# python feature_display.py --dataset ETT_h1 --dataset_fold_path data/point_rate01/ett_rate01_step48_point,data/point_rate05/ett_rate05_step48_point,data/point_rate09/ett_rate09_step48_point,data/block_rate05/ett_rate05_step48_block_blocklen6,data/subseq_rate05/ett_rate05_step48_subseq_seqlen36 --pdf_saving_path pdf_display --model_result_parent_fold pretrained_models/results_point_rate01/ETT_h1,pretrained_models/results_point_rate05/ETT_h1,pretrained_models/results_point_rate09/ETT_h1,pretrained_models/results_block_rate05/ETT_h1,pretrained_models/results_subseq_rate05/ETT_h1
    ...
    for model in model_lists:
        ...
        for i, (dataset_fold_path, model_result_parent_fold) in enumerate(zip(dataset_fold_paths, model_result_parent_folds)):
            ...
            for j in range(n_features):
                df = pd.DataFrame({"x": np.arange(0, test_X.shape[1]), "val": X_imputed[sample_idx, :, j]})
                df1 = pd.DataFrame({"x": np.arange(0, test_X.shape[1]), "val": X[sample_idx, :, j]})
                df2 = pd.DataFrame({"x": np.arange(0, test_X.shape[1]), "val": X_ori[sample_idx, :, j]})
                ...
                axes[i, j].set_xticks(list(np.arange(0, test_X.shape[1], max(10, test_X.shape[1]//10))))
                if i == 0:
                    axes[i, j].set_title(f"Feature #{j+1}")
                if j == 0:
                    # axes[i, j].set_ylabel(f"{dataset_name}")
                    axes[i, j].set_ylabel(f"{dataset_name.replace('ett_', '').replace('_step48', '').replace('_blocklen6', '').replace('_seqlen36', '')}")
