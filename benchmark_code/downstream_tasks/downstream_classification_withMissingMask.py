...
if __name__ == "__main__":
    ...
    train_missing_mask = (~np.isnan(pots_train_X)).astype(float)
    val_missing_mask = (~np.isnan(pots_val_X)).astype(float)
    test_missing_mask = (~np.isnan(pots_test_X)).astype(float)
    pots_train_X = np.concatenate([pots_train_X, train_missing_mask], axis=-1)
    pots_val_X = np.concatenate([pots_val_X, val_missing_mask], axis=-1)
    pots_test_X = np.concatenate([pots_test_X, test_missing_mask], axis=-1)
    train_X = np.concatenate([train_X, train_missing_mask], axis=-1)
    val_X = np.concatenate([val_X, val_missing_mask], axis=-1)
    test_X = np.concatenate([test_X, test_missing_mask], axis=-1)
