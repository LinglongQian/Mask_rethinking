...
if __name__ == "__main__":
    ...
    for n_round in range(5):
        ...
        print(n_flatten_features)
        print(pots_train_X.reshape(-1, n_flatten_features).shape)
        print(train_y.shape)
        print(train_X.shape)
        xgb = XGBClassifier()
        xgb.fit(
...
# def train(model, train_dataloader, val_dataloader, test_loader, n_classes):
#     n_epochs = 100
#     patience = 10
#     optimizer = torch.optim.Adam(model.parameters(), 1e-3)
#     current_patience = patience
...
#         val_loss = np.mean(loss_collector)
...
#     if n_classes == 2:
#         classification_metrics = calc_binary_classification_metrics(probability_collector, true_labels)
#         pr_auc = classification_metrics["pr_auc"]
#         roc_auc = classification_metrics["roc_auc"]
#     else:
#         pr_auc, roc_auc = None, None

#     logger.info(f"Final PR_AUC: {pr_auc}, ROC_AUC: {roc_auc}")
#     return probability_collector, pr_auc, roc_auc
...
#         )
#         simple_rnn_classifier = simple_rnn_classifier.to(args.device)
#         proba_predictions, pr_auc, roc_auc = train(
#             simple_rnn_classifier, train_loader, val_loader, test_loader, args.n_classes
#         )
#         rnn_pr_auc_collector.append(pr_auc)
#         rnn_roc_auc_collector.append(roc_auc)
...
#         transformer_classifier = transformer_classifier.to(args.device)
#         proba_predictions, pr_auc, roc_auc = train(
#             transformer_classifier, train_loader, val_loader, test_loader, args.n_classes
#         )
#         transformer_pr_auc_collector.append(pr_auc)
#         transformer_roc_auc_collector.append(roc_auc)
