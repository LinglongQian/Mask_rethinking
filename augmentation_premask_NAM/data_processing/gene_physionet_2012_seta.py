...
saving_dir = "data/physionet_2012_seta"

if __name__ == "__main__":
    data = tsdb.load("physionet_2012_seta")
    ...
    train_X_ori = train_X
    train_X = mcar_augmentation(train_X, ARTIFICIALLY_MISSING_RATE)
    ...
    train_X_flat = scaler.fit_transform(train_X.reshape(-1, train_X.shape[-1]))
    train_X_ori_flat = scaler.transform(train_X_ori.reshape(-1, train_X_ori.shape[-1]))
    train_X = train_X_flat.reshape(train_X.shape)
    train_X_ori = train_X_ori_flat.reshape(train_X_ori.shape)

    val_X_flat = scaler.transform(val_X.reshape(-1, val_X.shape[-1]))
    val_X_ori_flat = scaler.transform(val_X_ori.reshape(-1, val_X_ori.shape[-1]))
    val_X = val_X_flat.reshape(val_X.shape)
    val_X_ori = val_X_ori_flat.reshape(val_X_ori.shape)

    test_X_flat = scaler.transform(test_X.reshape(-1, test_X.shape[-1]))
    test_X_ori_flat = scaler.transform(test_X_ori.reshape(-1, test_X_ori.shape[-1]))
    test_X = test_X_flat.reshape(test_X.shape)
    test_X_ori = test_X_ori_flat.reshape(test_X_ori.shape)
